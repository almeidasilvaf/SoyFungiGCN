...
RUN apt-get update && apt-get install -y  git-core libcurl4-openssl-dev libgit2-dev libglpk-dev libgmp-dev libicu-dev libssl-dev libxml2-dev make pandoc pandoc-citeproc && rm -rf /var/lib/apt/lists/*
...
RUN Rscript -e 'remotes::install_version("ggnetwork",upgrade="never", version = "0.5.10")'
...
EXPOSE 3838
CMD  ["R", "-e", "library(SoyFungiGCN);options('shiny.port'=3838,shiny.host='0.0.0.0');SoyFungiGCN::run_app()"]
